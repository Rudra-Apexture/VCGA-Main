...
const StatsSection = ({ stats }) => {
    ...
    useEffect(() => {
        if (inView && !hasAnimated && countUpRefs.current.length === stats.length) {
            // Start animations if in view, not animated yet, and we have refs for all counters
            countUpRefs.current.forEach(ref => {
                if (ref && ref.start) { // make sure that ref is not null
                    ref.start();
                }
            });
